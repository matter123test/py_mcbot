...
class FabricMC(LoaderAPI):
    ...
    def get_latest_build_download_url(self, minecraft_version: str):
        loader_version = self.get_stable_fabric_loader_versions()[0]
        installer_version = self.get_latest_fabric_installer_version()

        url = self.server_url + f"/v2/versions/loader/{minecraft_version}/{loader_version}/{installer_version}/server/jar"
